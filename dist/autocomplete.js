...
export class GeocoderAutocomplete {
    ...
    geocoderRequestVersion = 0;
    geocoderRequestPending = false;
    /* Active place details request promise reject function */
    currentPlaceDetailsPromiseReject;
    placeDetailsRequestVersion = 0;
    placeDetailsRequestPending = false;
    ...
    categoryManager;
    placesRequestVersion = 0;
    placesRequestPending = false;
    lastEscKeyTime;
    DOUBLE_ESC_THRESHOLD = 500; // 500ms window for double-Esc
    destroyed = false;
    inputEventListener = (event) => this.onUserInput(event);
    keydownEventListener = (event) => this.onUserKeyPress(event);
    documentClickListener = (event) => this.onDocumentClick(event);
    clearButtonClickListener = (event) => this.clearFieldAndNotify(event);
    options = {
        limit: 5,
        placesLimit: 20,
        debounceDelay: 100
    ...
    validateFeatureCollectionResponse(data, responseName) {
        if (!data || !Array.isArray(data.features)) {
            throw new Error(`Invalid ${responseName} response: expected a features array`);
        }
        return data;
    }
    async sendGeocoderRequestOrAlt(currentValue) {
        let data;
    ...
    async sendPlacesRequest() {
        ...
        try {
            const bias = Object.keys(this.options.placesBias).length ? this.options.placesBias : PlacesApiHelper.convertGeocoderBiasToPlacesApiBias(this.options.bias);
            const filter = Object.keys(this.options.placesFilter).length ? this.options.placesFilter : PlacesApiHelper.convertGeocoderFilterToPlacesApiFilter(this.options.filter);
            data = await this.sendPlacesRequestOrAlt(categoryObj.keys, bias, filter, 0, this.options.placesLimit);
            if (requestVersion !== this.placesRequestVersion) {
                return;
            }
            const places = data.features || [];
